...
/*
************************************************************
        MODAL DE SELEÇÃO - FUNDOS ESTRUTURADOS
************************************************************

Modal com 6 combos para seleção de entidades e características do fundo estruturado.
Transfere valores selecionados para o form_fundos_estruturados.js principal.
...
- id_corretora *
- tipo_fundo * (select fixo: FII, FIAGRO, FI-Infra)
- segmento (select fixo: depende do tipo)
...
let modalSelecoesFundosEstruturados = null;

/**
 * 🏗️ Construtor do Modal de Seleções Fundos Estruturados
 */
export function construirModalSelecoesFundosEstruturados() {
    console.log('🏗️ Construindo Modal de Seleções Fundos Estruturados...');
    
    // Criar HTML do modal no DOM
    criarModalHTML();
    
    console.log('✅ Modal de Seleções Fundos Estruturados criado');
}
...
function fecharModal() {
    const modal = document.getElementById('modal-selecoes-fundos-estruturados');
    if (modal) {
...
function criarModalHTML() {
    ...
    if (document.getElementById('modal-selecoes-fundos-estruturados')) {
        return;
    }
    
    const modalHTML = `
        <div id="modal-selecoes-fundos-estruturados" style="display: none; position: fixed; z-index: 10000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5);">
            <div id="modal-selecoes-fundos-estruturados-content" style="position: absolute; top: 20%; left: 50%; transform: translateX(-50%); background: white; padding: 20px; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.3); min-width: 600px; max-height: 70vh; overflow-y: auto; cursor: move;">
                <h2 id="modal-selecoes-fundos-estruturados-header" style="margin: 0 0 20px 0; color: #333; cursor: move; user-select: none;">Selecionar Entidades do Fundo Estruturado</h2>
                <form id="form-selecoes-fundos-estruturados">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div>
                            <label style="display: block; margin-bottom: 5px; font-weight: bold;">Gestora *</label>
                            <select id="modal_id_gestora_est" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                                <option value="">Selecione...</option>
                            </select>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 5px; font-weight: bold;">Administradora *</label>
                            <select id="modal_id_administradora_est" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                                <option value="">Selecione...</option>
                            </select>
                        </div>
                        <div style="grid-column: 1 / -1;">
                            <label style="display: block; margin-bottom: 5px; font-weight: bold;">Banco Custodiante *</label>
                            <select id="modal_id_custodiante_est" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                                <option value="">Selecione...</option>
                            </select>
                        </div>
                        <div style="grid-column: 1 / -1;">
                            <label style="display: block; margin-bottom: 5px; font-weight: bold;">Corretora *</label>
                            <select id="modal_id_corretora_est" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                                <option value="">Selecione...</option>
                            </select>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 5px; font-weight: bold;">Tipo de Fundo *</label>
                            <select id="modal_tipo_fundo_est" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                                <option value="">Selecione...</option>
                                <option value="FII">FII - Fundo Imobiliário</option>
                                <option value="FIAGRO">FIAGRO - Fundo do Agronegócio</option>
                                <option value="FI-Infra">FI-Infra - Fundo de Infraestrutura</option>
                            </select>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 5px; font-weight: bold;">Segmento</label>
                            <select id="modal_segmento_est" style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                                <option value="">Selecione...</option>
                                <option value="Logística">Logística</option>
                                <option value="Shopping">Shopping</option>
                                <option value="Lajes Corporativas">Lajes Corporativas</option>
                                <option value="Híbrido">Híbrido</option>
                                <option value="Papel (CRI/CRA)">Papel (CRI/CRA)</option>
                    ...
                    <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px;">
                        <button type="button" id="btn-modal-fundos-est-cancelar" style="padding: 10px 20px; border: 1px solid #ccc; background: #f5f5f5; border-radius: 4px; cursor: pointer;">Cancelar</button>
                        <button type="button" id="btn-modal-fundos-est-ok" style="padding: 10px 20px; border: none; background: #007bff; color: white; border-radius: 4px; cursor: pointer;">OK</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    document.getElementById('btn-modal-fundos-est-cancelar').addEventListener('click', fecharModal);
    document.getElementById('btn-modal-fundos-est-ok').addEventListener('click', confirmarSelecao);
    
    // Clique no backdrop fecha modal
    document.getElementById('modal-selecoes-fundos-estruturados').addEventListener('click', (e) => {
        if (e.target.id === 'modal-selecoes-fundos-estruturados') {
            fecharModal();
        }
    });
    
    // ============= TORNAR MODAL ARRASTÁVEL =============
    const modalContent = document.getElementById('modal-selecoes-fundos-estruturados-content');
    const modalHeader = document.getElementById('modal-selecoes-fundos-estruturados-header');
...
async function popularCombosModal() {
    ...
    try {
        // Busca elementos
        const comboGestora = document.getElementById('modal_id_gestora_est');
        const comboAdministradora = document.getElementById('modal_id_administradora_est');
        const comboCustodiante = document.getElementById('modal_id_custodiante_est');
        const comboCorretora = document.getElementById('modal_id_corretora_est');
        ...
        if (comboGestora && gestoras?.dados?.dados) {
            ...
            console.warn('⚠️ Gestora NÃO populado');
        }
...
function validarCamposObrigatorios() {
    const gestora = document.getElementById('modal_id_gestora_est').value;
    const administradora = document.getElementById('modal_id_administradora_est').value;
    const custodiante = document.getElementById('modal_id_custodiante_est').value;
    const corretora = document.getElementById('modal_id_corretora_est').value;
    const tipoFundo = document.getElementById('modal_tipo_fundo_est').value;
...
function transferirValoresParaFormPrincipal() {
    const gestora = document.getElementById('modal_id_gestora_est');
    const administradora = document.getElementById('modal_id_administradora_est');
    const custodiante = document.getElementById('modal_id_custodiante_est');
    const corretora = document.getElementById('modal_id_corretora_est');
    const tipoFundo = document.getElementById('modal_tipo_fundo_est');
    const segmento = document.getElementById('modal_segmento_est');
    ...
    const evento = new CustomEvent('modal-fundos-estruturados-confirmado', {
        detail: { valores, textos }
    });
    document.dispatchEvent(evento);
    
    console.log('✅ Evento modal-fundos-estruturados-confirmado disparado');
}
...
export async function abrirModalNovo() {
    ...
    // Limpa campos
    document.getElementById('modal_id_gestora_est').value = '';
    document.getElementById('modal_id_administradora_est').value = '';
    document.getElementById('modal_id_custodiante_est').value = '';
    document.getElementById('modal_id_corretora_est').value = '';
    document.getElementById('modal_tipo_fundo_est').value = '';
    document.getElementById('modal_segmento_est').value = '';
...
export async function abrirModalComValores(valoresAtuais) {
    ...
    setTimeout(() => {
        ...
        if (valoresAtuais.id_gestora) {
            document.getElementById('modal_id_gestora_est').value = valoresAtuais.id_gestora;
        }
        if (valoresAtuais.id_administradora) {
            document.getElementById('modal_id_administradora_est').value = valoresAtuais.id_administradora;
        }
        if (valoresAtuais.id_custodiante) {
            document.getElementById('modal_id_custodiante_est').value = valoresAtuais.id_custodiante;
        }
        if (valoresAtuais.id_corretora) {
            document.getElementById('modal_id_corretora_est').value = valoresAtuais.id_corretora;
        }
        if (valoresAtuais.tipo_fundo) {
            document.getElementById('modal_tipo_fundo_est').value = valoresAtuais.tipo_fundo;
        }
        if (valoresAtuais.segmento) {
            document.getElementById('modal_segmento_est').value = valoresAtuais.segmento;
        }
